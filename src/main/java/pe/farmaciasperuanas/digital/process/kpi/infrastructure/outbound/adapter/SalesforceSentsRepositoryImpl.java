...
@Slf4j
public class SalesforceSentsRepositoryImpl implements SalesforceSentsRepository {

    private final ReactiveMongoTemplate reactiveMongoTemplate;

    @Override
    public Flux<SalesforceSents> findByDateRange(LocalDateTime startDate, LocalDateTime endDate) {
        Query query = new Query(Criteria.where("eventDate")
                .gte(startDate)
                .lte(endDate));
        return reactiveMongoTemplate.find(query, SalesforceSents.class);
    }

    @Override
    public Flux<SalesforceSents> findBySendId(Integer sendId) {
        Query query = new Query(Criteria.where("sendID").is(sendId));
        return reactiveMongoTemplate.find(query, SalesforceSents.class);
    }

    @Override
    public Flux<SalesforceSents> findByCorporacion(String corporacion) {
        Query query = new Query(Criteria.where("corporacion").is(corporacion));
        return reactiveMongoTemplate.find(query, SalesforceSents.class);
    }

    @Override
    public Flux<SalesforceSents> findByFechaProceso(LocalDate fechaProceso) {
